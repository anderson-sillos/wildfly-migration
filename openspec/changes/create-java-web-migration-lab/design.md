...
### 7. Persistência e Oracle por fase

O schema e os dados de teste Oracle permanecerão funcionalmente estáveis para não misturar migração de banco com migração de plataforma.

Nas fases 1 e 2, o laboratório usará o driver legado necessário para reproduzir o ambiente. No gate Java 17 da fase 3, ele adotará um driver mantido compatível com Java 17 e Oracle 19c. Nos gates WildFly 41, o servidor provisionará `com.oracle.database.jdbc:ojdbc17:23.26.2.0.0` e publicará `java:/jdbc/MigrationDS`; o driver não será empacotado no WAR.

Os mappers, aliases, type handlers e resultados MyBatis serão preservados e testados em todos os checkpoints públicos e gates internos. A suíte Oracle final será obrigatória para declarar a migração validada. Credenciais e URL serão fornecidas por ambiente ou secret local ignorado pelo controle de versão.

Alternativa considerada: empacotar o driver final no WAR. O provisionamento no servidor evita duplicação e mantém pool, credenciais e ciclo de vida sob controle do WildFly.
...
- [Um banco alternativo pode esconder diferenças de SQL Oracle] → Não considerar a migração validada sem a suíte de integração Oracle 19c.
...
- Qual distribuição licenciada ou fornecida pelo usuário será usada para reproduzir exatamente Java `1.7.0_80`?
