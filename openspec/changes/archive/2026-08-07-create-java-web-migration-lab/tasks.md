...
- [x] 3.25 Encerrar `CP-3E`: validar o runtime, o diagnóstico e o primeiro build Jakarta, registrar evidências e rollback e integrar o PR pelo commit `checkpoint(CP-3E): enter WildFly 41 and Jakarta EE 11`.
...
- [x] 3.34 Remover a ponte temporária de Log4j, definir explicitamente `logImpl=SLF4J` no MyBatis e integrar os logs ao mecanismo final do WildFly, comprovando no `server.log` as categorias dos mappers e exceções completas sem empacotar backend concorrente no WAR.
- [x] 3.35 Encerrar `CP-3G`: executar contratos web, validar descoberta, fachada, conteúdo e registro do JAR interno do SCI, executar auditoria de dependências e segurança, registrar evidências e rollback e integrar o PR pelo commit `checkpoint(CP-3G): replace legacy web libraries`.

### Checkpoint CP-3H — Oracle e auditoria final

- [x] 3.36 Fixar MyBatis 3.5.19, XMLBeans 5.3.0 e dom4j 2.2.0 e reexecutar geração e testes XML seguros.
...
- [x] 3.48 Capturar diferenças exclusivas do JDK 25 e aplicar o menor conjunto de correções sem alterar o contrato funcional.
- [x] 3.49 Executar as trilhas H2 e Oracle no OpenJDK 25 e a qualificação adicional no OpenJDK 21, incluindo contratos, empacotamento, segredos, portas e proveniência open source.
- [x] 3.50 Encerrar `CP-3J`: validar o destino OpenJDK/WildFly comunitário com evidências `portable-ci` e `oracle-qualified`, registrar rollback e integrar o PR pelo commit `checkpoint(CP-3J): qualify OpenJDK 25`.
...
- [x] 3.52 Gerar relatório consolidado com as três fases, checkpoints parciais, gates, versões, estados `portable-ci` e `oracle-qualified`, exceções resolvidas e limitações.
- [x] 3.53 Reproduzir o destino final a partir de checkout limpo usando documentação, `doctor`, OpenJDK, WildFly comunitário, H2 em memória e configuração Oracle externa.
- [x] 3.54 Auditar histórico Git, rastreabilidade de PRs e commits, segredos, licenças, checksums, dependências, WAR e instruções de rollback.
- [x] 3.55 Encerrar `CP-3K`: aprovar todas as evidências H2 e Oracle, integrar o PR pelo commit `checkpoint(CP-3K): complete final destination` e criar a tag `migration/03-final`.
