...
| `org.apache.xmlbeans:xmlbeans:2.3.0` | `org.apache.xmlbeans:xmlbeans:5.3.0` (aplicado em 3.11) | Sim; requer Java 8 ou superior | Neutro | **Alto.** Tipos regenerados a partir do XSD e round-trip de serialização, namespaces e validação aprovados na atividade 3.11. A API Log4j 2 introduzida transitivamente não é um backend; `log4j-core` permanece proibido no WAR. |
| `dom4j:dom4j:1.6.1` | `org.dom4j:dom4j:2.2.0` (aplicado em 3.12) | Sim; requer Java 11 | Neutro | **Alto.** A coordenada mudou, o parser seguro foi preservado e os casos legítimo, XXE e expansão de entidades foram aprovados. Nenhuma transitiva opcional foi habilitada. |
| `xml-apis:xml-apis:1.3.02` | remover | Sim, por meio do módulo `java.xml` do JDK | Neutro | **Médio.** Removido em 3.13; DOM, SAX e JAXP foram comprovados no módulo `java.xml`, sem duplicação no WAR. |
| `org.apache.geronimo.specs:geronimo-stax-api_1.0_spec:1.0` | remover | Sim, por meio do módulo `java.xml` do JDK | Neutro | **Médio.** Removido em 3.13 junto de `stax-api:1.0.1`; `javax.xml.stream` foi comprovado no módulo `java.xml`. |
...
| `org.apache.logging.log4j:log4j-api:2.24.2` | manter somente como API transitiva do XMLBeans 5.3.0 | É API, não backend. A atividade 3.11 confirmou a presença da API e a ausência de `log4j-core` no WAR. |
| `stax:stax-api:1.0.1` | removido pela atualização do XMLBeans | Deixou de vir transitivamente com XMLBeans 5.3.0; as APIs XML diretas restantes serão removidas em 3.13. |
