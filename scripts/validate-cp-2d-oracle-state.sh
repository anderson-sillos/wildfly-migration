...
--compile-only não exige driver, credenciais ou conexão.
USAGE
}

fail() {
  printf 'FALHA CP-2D estado Oracle: %s\n' "$1" >&2
  exit 1
}

cleanup() {
  case "$TEMP_DIRECTORY" in
    "${TMPDIR:-/tmp}"/wildfly-migration-cp2d-oracle-state.*)
...
if [[ "$COMPILE_ONLY" == true ]]; then
  printf 'OK: sonda de estado Oracle CP-2D compilada sem driver ou credenciais\n'
  exit 0
fi
...
printf 'OK: estado Oracle da fase 2 comparado ao baseline e vinculado ao WAR\n'
