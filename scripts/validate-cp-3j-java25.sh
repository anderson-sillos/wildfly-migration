...
for path in "$POM" "$BUILD" "$SMOKE" "$WORKFLOW" "$MANIFEST"; do
...
for marker in \
  ...
  'build-cp-3j-java25.sh' \
  'smoke-wildfly41-datasource.sh' \
  '--java 25'; do
...
printf 'OK: CP-3J/3.47 troca somente a JVM para OpenJDK 25 e mantém Jakarta/bytecode 21\n'
